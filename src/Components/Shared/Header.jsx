        ...
        <div className="flex items-center">
          <div
            className={`flex items-center flex-col gap-3 md:flex-row md:static absolute ${
              menu ? "-top-80 right-16" : " right-16 bg-stone-400 md:bg-white px-4 py-2 rounded-lg top-10" 
            } duration-500`}
          >
            <Nav menu={menu} />
            {!user ? (
              <div>
                <Link to={"/login"}>
                  <button className="btn btn-info hover:bg-gray-500 hover:text-white text-yellow-50 md:ml-10">
                    {" "}
                    Sign in
                  </button>
                </Link>
